...
import { MediumIssues } from "../src/issues";

const issueType = "M";

describe('test medium issues', function () {
    ...
    describe('run tests', async function () {
        ...
        for(const contract of contracts){
            ...
            let issue: Issue = {name: "", regexOrAST: 'Regex', type: IssueTypes.M, title: '', regex: / /g};

            //this is only if we don't have corresponding contracts for all issues
            for(const mediumIssue of MediumIssues){
                if(mediumIssue.name === issueName){
                    issue = mediumIssue;
                    break;
                }
            }
